...
public class LoginActivity extends AppCompatActivity implements LoaderCallbacks<Cursor> {
    ...
    private static final int REQUEST_ACCESS_INTERNET = 1;
    private static final int REQUEST_ACCESS_STOCKAGE = 2;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        url_base = getString(R.string.url_server);
    ...
    private boolean mayAcessInternet() {
        if (checkSelfPermission(INTERNET) == PackageManager.PERMISSION_GRANTED) {
            return true;
        }
        if (shouldShowRequestPermissionRationale(INTERNET)) {
            Snackbar.make(mEmailView, R.string.permission_rationale, Snackbar.LENGTH_INDEFINITE)
                    .setAction(android.R.string.ok, v -> requestPermissions(new String[]{INTERNET}, REQUEST_ACCESS_INTERNET));
        } else {
    ...
    private boolean mayAcessStockage() {
        if (checkSelfPermission(WRITE_EXTERNAL_STORAGE) == PackageManager.PERMISSION_GRANTED) {
            return true;
        }
        if (shouldShowRequestPermissionRationale(WRITE_EXTERNAL_STORAGE)) {
            Snackbar.make(mEmailView, R.string.permission_rationale, Snackbar.LENGTH_INDEFINITE)
                    .setAction(android.R.string.ok, v -> requestPermissions(new String[]{WRITE_EXTERNAL_STORAGE}, REQUEST_ACCESS_INTERNET));
        } else {
            requestPermissions(new String[]{WRITE_EXTERNAL_STORAGE}, REQUEST_ACCESS_STOCKAGE);
        }
        return false;
    }
    ...
    public class UserLoginTask extends AsyncTask<Void, Void, Boolean> {
        ...
        protected Boolean doInBackground(Void... params) {
            ...
            try {
                ...
                try {
                    toSendData.accumulate("username", mEmail);
                    toSendData.accumulate("password", mPassword);
                } catch (JSONException e) {
                    e.printStackTrace();
                    return false;
                }
